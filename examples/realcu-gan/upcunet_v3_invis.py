...
from invis import nn as nn
from invis.nn import functional as F
import os
import numpy as np
...
def dq(inp, if_half, cache_mode, delta, minn, device):
    ...
    inp = inp.half() if if_half else inp.float()
    return inp / 255 * delta + minn
...
class UpCunet2x(nn.Module):
    ...
    def forward(self, x, tile_mode, cache_mode, alpha, pro):
        ...
        se_mean1 /= n_patch
        res = torch.zeros(
            (n, c, h * 2 - 72, w * 2 - 72), dtype=torch.uint8, device=x.device
        )
        for i in range(0, h - 36, crop_size[0]):
            for j in range(0, w - 36, crop_size[1]):
        ...
        for i in range(0, h - 36, crop_size[0]):
            for j in range(0, w - 36, crop_size[1]):
                tmp0, x_crop = self.unet1.forward_a(
                    x[:, :, i : i + crop_size[0] + 36, j : j + crop_size[1] + 36]
                )
                if if_half:  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        x_crop.float(), dim=(2, 3), keepdim=True
...
class UpCunet4x(nn.Module):
    ...
    def forward(self, x, tile_mode, cache_mode, alpha, pro):
        ...
        se_mean1 /= n_patch
        res = torch.zeros(
            (n, c, h * 4 - 152, w * 4 - 152), dtype=torch.uint8, device=x.device
        )
        for i in range(0, h - 38, crop_size[0]):
            for j in range(0, w - 38, crop_size[1]):
                x, tmp_x1, tmp_x4 = tmp_dict[i][j]
                ...
                x[:, :, : h2 * 4, : w2 * 4] += F.interpolate(
                    x00_crop, scale_factor=4, mode="nearest"
                )
                if pro:
                    res[
    ...
    def forward_gap_sync(self, x, tile_mode, alpha, pro):
        ...
        for i in range(0, h - 38, crop_size[0]):
            for j in range(0, w - 38, crop_size[1]):
                tmp0, x_crop = self.unet1.forward_a(
                    x[:, :, i : i + crop_size[0] + 38, j : j + crop_size[1] + 38]
                )
                if if_half:  # torch.HalfTensor/torch.cuda.HalfTensor
                    tmp_se_mean = torch.mean(
                        x_crop.float(), dim=(2, 3), keepdim=True
        ...
        for i in range(0, h - 38, crop_size[0]):
            for j in range(0, w - 38, crop_size[1]):
                ...
                x00_crop = x00[:, :, i : i + h1 - 38, j : j + w1 - 38]
                _, _, h2, w2 = x00_crop.shape
                x_crop[:, :, : h2 * 4, : w2 * 4] += F.interpolate(
                    x00_crop, scale_factor=4, mode="nearest"
                )
                if pro:
                    res[
...
class RealWaifuUpScaler(object):
    ...
    def np2tensor(self, np_frame):
        tensor = torch.from_numpy(
            np.transpose(np_frame, (2, 0, 1))
        ).unsqueeze(0).to(self.device)
        tensor = tensor.half() if self.half else tensor.float()
        return tensor / (255 / 0.7) + 0.15 if self.pro else tensor / 255
    ...
    def __call__(self, frame, tile_mode, cache_mode, alpha):
        with torch.no_grad():
            ...
            if cache_mode == 3:
                result = self.model.forward_gap_sync(tensor, tile_mode, alpha, self.pro)
            elif cache_mode == 2:
                result = self.model.forward_fast_rough(tensor, tile_mode, alpha, self.pro)
            else:
                result = self.model(tensor, tile_mode, cache_mode, alpha, self.pro)
            return self.tensor2np(result)
...
def main():
    ...
    for weight_path, scale in [
        ("weights_v3/up2x-latest-denoise3x.pth", 2),
        ("weights_v3/up3x-latest-denoise3x.pth", 3),
        ("weights_v3/up4x-latest-denoise3x.pth", 4),
        # ("weights_pro/pro-denoise3x-up2x.pth", 2),
            ...
            for cache_mode in [0, 1, 2, 3]:
                for alpha in [1]:
                    ...
                    for name in os.listdir(input_dir):
                        ...
                        tmp_path = os.path.join(tmp_dir, "%s.%s" % (int(time() * 1000000), suffix),)
                        ...
                        t0 = time()
                        result = upscaler2x(
                            frame, tile_mode=tile_mode, cache_mode=cache_mode, alpha=alpha,
                        )[:, :, ::-1]
                        t1 = time()
                        print(prefix, "done", t1 - t0, "tile_mode=%s" % tile_mode, cache_mode)
                        tmp_opt_path = os.path.join(
                            root_path, "tmp", "%s.%s" % (int(time() * 1000000), suffix),
                        )
                        ...
                        while 1:
                            ...
                            if not os.path.exists(os.path.join(output_dir, prefix + suffix)):
                                break
                            else:
                                n += 1
