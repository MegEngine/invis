...
if __name__ == "__main__":
    ...
    from densenet_invis import densenet161 as mge_densenet

    model = densenet161(pretrained=True)
    mge_model = mge_densenet(pretrained=True)
    model.eval()
    mge_model.eval()

    print(
        "Check densenet value:",
        check_output(mge_model, model, input_shape=(1, 3, 224, 224), rtol=4e-3)
    )
