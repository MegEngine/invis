...
if __name__ == "__main__":
    ...
    from googlenet_invis import googlenet as mge_googlenet

    model = googlenet(pretrained=True)
    mge_model = mge_googlenet(pretrained=True)
    model.eval()
    mge_model.eval()

    print(
        "Check densenet value:",
        check_output(mge_model, model, input_shape=(1, 3, 224, 224), rtol=5e-3)
    )
